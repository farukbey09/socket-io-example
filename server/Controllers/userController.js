...
const registerUser = async (req, res) => {
    try {
        ...
        if (!name || !email || !password) return res.status(500).json("all fields are required")
...
const loginUser = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            if (!isValidPassword) return res.status(400).json("Password or Email is wrong...")
            else {
                const token = createToken(user._id)
...
const findUser = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            res.status(400).json("user didn't found")
        }
